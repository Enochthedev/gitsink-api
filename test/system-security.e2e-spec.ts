...
describe('System Security Tests (e2e)', () => {
  ...
  describe('Authentication Security', () => {
    ...
    it('should prevent timing attacks on authentication', async () => {
      ...
      const avgInvalidTime =
        invalidEmailTimes.reduce((a, b) => a + b, 0) / invalidEmailTimes.length;
  ...
  describe('Authorization Security', () => {
    ...
    beforeAll(async () => {
      ...
      const user1Response = await request(app.getHttpServer()).post('/auth/signup').send({
        email: 'authz-user1@example.com',
        username: 'authz-user1',
        password: 'SecurePassword123!',
      });

      const user2Response = await request(app.getHttpServer()).post('/auth/signup').send({
