...
export class BitbucketOAuthController {
  private readonly logger = new Logger(BitbucketOAuthController.name);
  ...
  async authorize(@Res() res: Response) {
    try {
      ...
      const clientId = this.config.get<string>('BITBUCKET_CLIENT_ID');
      const redirectUri = this.config.get<string>('BITBUCKET_REDIRECT_URI');
      const scopes = ['repositories', 'account'];
      const state = this.generateState();

      if (!clientId || !redirectUri) {
        return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
          error: 'Bitbucket OAuth configuration missing',
        });
      }
      ...
      this.logger.error('Bitbucket authorization failed:', error);
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
  ...
  async callback(
    ...
    try {
      ...
      const clientId = this.config.get<string>('BITBUCKET_CLIENT_ID');
      const clientSecret = this.config.get<string>('BITBUCKET_CLIENT_SECRET');
      const redirectUri = this.config.get<string>('BITBUCKET_REDIRECT_URI');
      ...
      // Authenticate with Bitbucket to get user info
      ...
      this.logger.error('Bitbucket OAuth callback failed:', error);
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
