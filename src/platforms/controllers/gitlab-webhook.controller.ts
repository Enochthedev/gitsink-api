...
export class GitLabWebhookController {
    ...
    private async handlePushEvent(payload: GitLabWebhookPayload): Promise<void> {
        this.logger.debug(`Processing GitLab push event for project: ${payload.project.path_with_namespace}`);

        // Check if Portfolio.md was modified
        const portfolioModified = payload.commits.some(commit =>
        ...
        if (portfolioModified) {
            this.logger.log(`Portfolio.md modified in ${payload.project.path_with_namespace}, triggering sync`);
    ...
    private async handleMergeRequestEvent(payload: any): Promise<void> {
        this.logger.debug(`Processing GitLab merge request event for project: ${payload.project?.path_with_namespace}`);
        // TODO: Handle merge request events if needed for project sync
    }

    private async handleIssueEvent(payload: any): Promise<void> {
        this.logger.debug(`Processing GitLab issue event for project: ${payload.project?.path_with_namespace}`);
        // TODO: Handle issue events if needed for project statistics
    }

    private async handleNoteEvent(payload: any): Promise<void> {
        this.logger.debug(`Processing GitLab note event for project: ${payload.project?.path_with_namespace}`);
        // TODO: Handle comment events if needed
    }

    private async handlePipelineEvent(payload: any): Promise<void> {
        this.logger.debug(`Processing GitLab pipeline event for project: ${payload.project?.path_with_namespace}`);
        // TODO: Handle pipeline events if needed for project status
    }

    private async handleJobEvent(payload: any): Promise<void> {
        this.logger.debug(`Processing GitLab job event for project: ${payload.project?.path_with_namespace}`);
        // TODO: Handle job events if needed
    }

    private async handleTagPushEvent(payload: any): Promise<void> {
        this.logger.debug(`Processing GitLab tag push event for project: ${payload.project?.path_with_namespace}`);
        // TODO: Handle tag creation for release tracking
    }

    private async handleWikiPageEvent(payload: any): Promise<void> {
        this.logger.debug(`Processing GitLab wiki page event for project: ${payload.project?.path_with_namespace}`);
        // TODO: Handle wiki changes if needed
    }

    private async handleDeploymentEvent(payload: any): Promise<void> {
        this.logger.debug(`Processing GitLab deployment event for project: ${payload.project?.path_with_namespace}`);
        // TODO: Handle deployment events if needed
    }

    private async handleFeatureFlagEvent(payload: any): Promise<void> {
        this.logger.debug(`Processing GitLab feature flag event for project: ${payload.project?.path_with_namespace}`);
        // TODO: Handle feature flag events if needed
    }

    private async handleReleaseEvent(payload: any): Promise<void> {
        this.logger.debug(`Processing GitLab release event for project: ${payload.project?.path_with_namespace}`);
