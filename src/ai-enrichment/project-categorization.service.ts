...
export class ProjectCategorizationService {
  ...
  private checkFilePatterns(content: RepositoryContent, patterns: string[]): number {
    ...
    for (const pattern of patterns) {
      const hasMatch = content.files.some(
        file =>
          file &&
          file.name &&
          file.path &&
  ...
  private generateFeatureTags(content: RepositoryContent): string[] {
    ...
    const hasDocumentation = content.files.some(
      f => f.path.includes('docs') || f.name.includes('README'),
    );
  ...
  filterProjectsByCategory(
    ...
    return projects.filter(project => {
      ...
      if (tags && tags.length > 0) {
        const hasMatchingTag = tags.some(tag => project.category.tags?.includes(tag));
  ...
  searchProjectsByCategory(projects: any[], searchTerm: string): any[] {
    ...
    return projects.filter(project => {
      ...
      if (
        project.category.secondary?.some((sec: string) =>
      ...
      // Search in tags
      if (
        project.category.tags?.some((tag: string) => tag.toLowerCase().includes(lowerSearchTerm))
      ) {
        return true;
      }
