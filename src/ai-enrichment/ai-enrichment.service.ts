...
export class AIEnrichmentService {
  ...
  async analyzeRepository(
    ...
    try {
      ...
      if (!forceReanalysis) {
        ...
        if (
          existingAnalysis &&
          this.isAnalysisRecent(existingAnalysis.createdAt)
        ) {
      ...
      const technologyStack =
        this.extractResult(technologies, 'technology detection') ||
        this.getEmptyTechnologyStack();
      const generatedDescription = this.extractResult(
        description,
        'description generation',
      );
      const projectCategory =
        this.extractResult(category, 'project categorization') ||
      ...
      const keyFeatures = this.extractKeyFeatures(
        repositoryContent,
        technologyStack,
      );
      ...
      const analysisResult: AIAnalysisResult = {
        description:
          generatedDescription?.description || 'No description available',
      ...
      this.logger.error(
        `Failed to analyze repository for project ${projectId}:`,
        error,
      );
      const errorMessage =
   ...
   */
  private async analyzeTechnologies(
   ...
   */
  private async categorizeProject(
    content: RepositoryContent,
  ...
  private generateSuggestedTags(
    ...
    // Add platform tags
    technologies?.platforms?.forEach((platform) =>
  ...
  private extractKeyFeatures(
    ...
    if (
      content.files.some(
        (f) => f.content?.includes('graphql') || f.name.includes('graphql'),
      )
    ) {
    ...
    // Testing features
    if (technologies.testingFrameworks.length > 0 ||
      content.files.some(f =>
        f.path.includes('test') ||
        f.path.includes('spec') ||
        f.name.includes('test') ||
    ...
    // Deployment features
    if (
      content.files.some((f) => f.name.toLowerCase().includes('dockerfile'))
    ) {
  ...
  private loadConfiguration(): EnrichmentConfig {
    return {
      ...
      enableAIDescription: this.configService.get<boolean>(
      ...
      enableCategorization: this.configService.get<boolean>(
      ...
      maxFilesToAnalyze: this.configService.get<number>(
