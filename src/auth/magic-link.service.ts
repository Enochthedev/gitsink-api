...
export class MagicLinkService {
    ...
    async sendMagicLink(
        ...
        try {
            ...
            if (!this.isValidEmail(email)) {
                this.magicLinkFailuresCounter.inc({ operation, reason: 'invalid_email' });
            ...
            if (!user) {
                ...
                this.magicLinkFailuresCounter.inc({ operation, reason: 'user_not_found' });
            ...
            if (recentTokens >= 3) {
                this.magicLinkFailuresCounter.inc({ operation, reason: 'rate_limited' });
    ...
    async validateMagicLink(
        ...
        try {
            ...
            if (!token || token.length !== 64) { // 32 bytes = 64 hex chars
                this.magicLinkFailuresCounter.inc({ operation, reason: 'invalid_format' });
            ...
            if (!validToken) {
                this.magicLinkFailuresCounter.inc({ operation, reason: 'invalid_token' });
     ...
     */
    async cleanupExpiredTokens(email?: string): Promise<number> {
