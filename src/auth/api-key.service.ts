...
export class ApiKeyService {
    ...
    async generateApiKey(
        ...
        try {
            ...
            if (!user) {
                this.apiKeyOperationsCounter.inc({ operation, status: 'failure', tier: 'unknown' });
            ...
            this.apiKeyOperationsCounter.inc({ operation, status: 'success', tier: user.tier });
    ...
    async validateApiKey(
        ...
        try {
            ...
            if (!apiKey || apiKey.length < 32) {
                this.apiKeyValidationCounter.inc({ result: 'invalid_format', tier: 'unknown' });
                return {
                    user: null as any,
                    isValid: false,
            ...
            // Check each user's API key hash (constant time comparison)
            ...
            if (!validUser) {
                this.apiKeyValidationCounter.inc({ result: 'invalid', tier: 'unknown' });
            ...
            if (rateLimitResult.exceeded) {
                this.apiKeyValidationCounter.inc({ result: 'rate_limited', tier: validUser.tier });
            ...
            const operationDuration = (Date.now() - operationStart) / 1000;
            this.apiKeyValidationCounter.inc({ result: 'valid', tier: validUser.tier });
            this.apiKeyOperationDuration.observe({ operation }, operationDuration);
