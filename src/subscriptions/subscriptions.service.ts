...
export class SubscriptionsService {
     ...
     */
    async upgradeTier(userId: string, dto: UpgradeTierDto): Promise<{
        checkoutUrl?: string;
     ...
     * Cancel subscription
     */
    async cancelSubscription(userId: string, reason?: string, immediate: boolean = false): Promise<SubscriptionResponseDto> {
        const subscription = await this.prisma.subscription.findUnique({
            where: { userId },
            include: { tier: true },
        });
     ...
     */
    async checkLimit(userId: string, resource: 'projects' | 'apiKeys' | 'apiCalls' | 'storage', increment: number = 1): Promise<{ allowed: boolean; current: number; limit: number; message?: string }> {
        const subscription = await this.prisma.subscription.findUnique({
            where: { userId },
            include: { tier: true },
        });
        ...
        switch (resource) {
            case 'projects':
                ...
                break;
            case 'apiKeys':
                current = await this.prisma.apiKeyToken.count({ where: { userId, revokedAt: null, isActive: true } });
    ...
    private toSubscriptionResponse(subscription: any): SubscriptionResponseDto {
        ...
        return {
            ...
            tier: {
                ...
                monthlyPrice: tier.monthlyPrice instanceof Decimal ? tier.monthlyPrice.toNumber() : Number(tier.monthlyPrice),
                yearlyPrice: tier.yearlyPrice instanceof Decimal ? tier.yearlyPrice.toNumber() : Number(tier.yearlyPrice),
