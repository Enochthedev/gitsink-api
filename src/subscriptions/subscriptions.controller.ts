...
export class SubscriptionsController {
    ...
    @Get('tiers')
    @ApiOperation({ summary: 'List all available tiers', description: 'Get all subscription tiers with pricing and features' })
    ...
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Get current subscription', description: 'Get the authenticated user\'s current subscription' })
    @ApiResponse({ status: 200, description: 'Current subscription', type: SubscriptionResponseDto })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    async getCurrentSubscription(@Req() req: RequestWithUser): Promise<SubscriptionResponseDto | null> {
    ...
    @UseGuards(EnhancedJwtGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Get usage statistics', description: 'Get current usage vs limits for the billing period' })
    ...
    @UseGuards(EnhancedJwtGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Upgrade subscription tier', description: 'Upgrade to a new subscription tier' })
    ...
    @UseGuards(EnhancedJwtGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Resume subscription', description: 'Resume a canceled subscription before period end' })
    ...
    @UseGuards(EnhancedJwtGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Check feature access', description: 'Check if user has access to a specific feature' })
