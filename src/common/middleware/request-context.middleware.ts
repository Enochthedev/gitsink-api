  ...
  use(req: RequestWithContext, res: Response, next: NextFunction) {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      try {
        ...
        this.logger.logAuthentication('token_validation', true, 'jwt', {
          requestId: req.requestId,
          correlationId: req.correlationId,
        });
      } catch (error) {
        this.logger.logAuthentication('token_validation', false, 'jwt', {
          requestId: req.requestId,
          correlationId: req.correlationId,
          metadata: {
...
export class SecurityMiddleware implements NestMiddleware {
  ...
  use(req: RequestWithContext, res: Response, next: NextFunction) {
    ...
    if (threats.length > 0) {
      this.logger.logSecurity('potential_security_threat_detected', {
        requestId: req.requestId,
        correlationId: req.correlationId,
        ip: req.ip,
        userAgent,
  ...
  private isSuspiciousUserAgent(userAgent: string): boolean {
    const suspiciousAgents = ['sqlmap', 'nikto', 'nmap', 'masscan', 'burp', 'owasp', 'scanner'];
  ...
  private calculateThreatSeverity(threats: string[]): 'low' | 'medium' | 'high' | 'critical' {
    if (
      threats.includes('suspicious_request_body') ||
      threats.includes('suspicious_query_parameters')
    ) {
      return 'high';
    }
