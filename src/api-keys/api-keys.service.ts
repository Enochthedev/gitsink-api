...
export class ApiKeysService {
     ...
     * Update an API key
     */
    async updateApiKey(userId: string, keyId: string, dto: UpdateApiKeyDto): Promise<ApiKeyResponseDto> {
        const key = await this.prisma.apiKeyToken.findFirst({
            where: { id: keyId, userId, revokedAt: null },
        });
     ...
     */
    async validateApiKey(apiKey: string): Promise<{ userId: string; permissions: string[]; keyId: string } | null> {
        const keyHash = this.hashApiKey(apiKey);
        ...
        this.prisma.apiKeyToken.update({
            where: { id: key.id },
            data: {
                lastUsedAt: new Date(),
