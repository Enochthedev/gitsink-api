...
export class BillingController {
    ...
    @ApiQuery({ name: 'provider', required: false, description: 'Payment provider (default: stripe)' })
    ...
    @UseGuards(EnhancedJwtGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Get payment history', description: 'List all payments made by the user' })
    ...
    @ApiOperation({ summary: 'LemonSqueezy webhook handler', description: 'Handle LemonSqueezy webhook events' })
    @ApiResponse({ status: 200, description: 'Webhook processed' })
    @ApiResponse({ status: 400, description: 'Invalid signature' })
