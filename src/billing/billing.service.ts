...
export class BillingService {
    ...
    constructor(
        ...
        this.defaultSuccessUrl = this.configService.get<string>('BILLING_SUCCESS_URL')
            || 'http://localhost:3000/billing/success';
        this.defaultCancelUrl = this.configService.get<string>('BILLING_CANCEL_URL')
    ...
    async createPortalSession(userId: string, provider: string): Promise<BillingPortalResponseDto> {
        ...
        const session = await paymentProvider.createPortalSession({
            ...
            returnUrl: this.configService.get<string>('BILLING_RETURN_URL') || 'http://localhost:3000/settings',
        });
    ...
    private getPriceIdForTier(
        ...
        switch (provider) {
            ...
            case 'lemonsqueezy':
                return cycle === 'yearly' ? tier.lemonSqueezyVariantIdYearly : tier.lemonSqueezyVariantIdMonthly;
            default:
                return null;
        }
    }
